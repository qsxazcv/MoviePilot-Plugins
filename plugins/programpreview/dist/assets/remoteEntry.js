...
let moduleMap = {
  "./Config":()=>{dynamicLoadingCss(["__federation_expose_Config-programpreview.css"], false, './Config');return __federation_import('./__federation_expose_Config-programpreview.js').then(module =>Object.keys(module).every(item => exportSet.has(item)) ? () => module.default : () => module)},
  "./Page":()=>{dynamicLoadingCss(["__federation_expose_Page-programpreview.css"], false, './Page');return __federation_import('./__federation_expose_Page-programpreview.js').then(module =>Object.keys(module).every(item => exportSet.has(item)) ? () => module.default : () => module)},
};
const seen = {};
const dynamicLoadingCss = (cssFilePaths, dontAppendStylesToHead, exposeItemName) => {const metaUrl = import.meta.url;if (typeof metaUrl === 'undefined') return;const curUrl = metaUrl.substring(0, metaUrl.lastIndexOf('remoteEntry.js'));cssFilePaths.forEach(cssPath => {const href = curUrl + cssPath;if (dontAppendStylesToHead) { const key = 'css__programpreview__' + exposeItemName; window[key] = window[key] || []; window[key].push(href); return; }if (href in seen) return; seen[href] = true;const element = document.createElement('link'); element.rel = 'stylesheet'; element.href = href; document.head.appendChild(element);});};
