...
def _fetch_115_qrcode_image(uid: str) -> bytes:
    if not uid:
        raise ValueError("二维码 uid 不能为空")

    qrcode_url = "https://qrcodeapi.115.com/api/1.0/web/1.0/qrcode?uid=" + quote(
        uid, safe=""
    )
    request = URLRequest(
        qrcode_url,
    ...
    if not image or not image.startswith(b"\x89PNG\r\n\x1a\n"):
        raise ValueError(f"二维码接口返回非 PNG 内容: {content_type or 'unknown'}")
    return image
