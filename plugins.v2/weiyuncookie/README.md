...
- 配置页和详情页采用 Vue 联邦 UI，按 z256 风格提供左侧导航、二级标签、卡片化分区、sticky toolbar 与明确空状态。
...
- 修复二维码图片链接过早打开显示 `qrcode not ready`。
- 扫码任务运行中，图片接口会短暂等待二维码生成后再返回。
- 未就绪或登录完成后返回友好 SVG 占位图，不再显示纯文本错误。


### v0.1.26

- 浏览器模式命名调整：`插件内置` 改为 `Playwright Chromium`，`兼容模式` 改为 `CloakBrowser`，让配置含义更直观。


### v0.1.25

- 新增浏览器预热开关。
- 插件启用后后台预热 Chromium / CloakBrowser，减少首次扫码冷启动等待。
- 预热失败时自动回退到扫码时正常启动，不影响原有登录流程。


### v0.1.24

- 优化启动扫码后的二维码弹出速度。
- 缩短浏览器页面等待、二维码就绪检测和截图超时。
- 提升前端状态轮询频率，二维码生成后更快显示。


### v0.1.23

- 按 z2561221/MoviePilot-Plugins 设计语言重构为 Vue 联邦 UI。
- 配置页采用左侧导航、二级标签、卡片化分区和统一操作区。
- 详情页采用 sticky toolbar、状态卡片、二维码与 Cookie 管理面板。
