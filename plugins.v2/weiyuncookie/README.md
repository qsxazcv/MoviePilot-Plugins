...
- 支持 QQ / 微信扫码登录微云。
- 自动提取并保存 Cookie，插件页默认隐藏 Cookie 明文。
- 主页展示保存状态、Cookie 数量、最近检测和 OpenList 同步结果。
...
- 优化 Cookie 复制逻辑，点击复制时重新读取最新状态。
- 通过浏览器 copy 事件直接写入剪贴板，减少提示成功但剪贴板为空的问题。
- 近期结果改为紧凑网格展示，降低页面高度占用。

### v0.1.29

- 修复部分浏览器或嵌入页面中 Cookie 复制失败的问题。
- 复制按钮增加隐藏 textarea 兜底复制，仍不在页面展示 Cookie 明文。

### v0.1.28

- 隐藏主页与配置页完整 Cookie 明文展示，避免截图或远程查看时泄露登录态。
- 详情页改为 Cookie 管理隐私面板，仅展示保存状态、数量和最近结果。
- 保留复制 Cookie、立即检测、OpenList 同步和清除 Cookie 等操作。

### v0.1.27

- 优化 UI 排序与信息层级，减少不必要的卡片化展示。
- 配置页改为“常用设置 / 登录环境 / 自动检测与通知 / OpenList 同步 / 运行档案”的分区表单。
- 详情页移除冗余功能卡片，改为状态概览、二维码操作区、Cookie 管理面板和近期结果列表。

### v0.1.26

- 按四大平台节目预告风格重设计配置页与详情页。
- 配置页改为控制台头部、状态卡片、功能卡片和分组设置卡片。
- 详情页统一为状态网格、实时状态卡片、二维码操作区和 Cookie 管理面板。
- 修复 Vue 联邦前端中文显示乱码问题。
