...
YOUKU_SHORT_DRAMA_CHANNELS = [
    ('shortdrama', 'https://www.youku.com/ku/webshortdrama'),
]
...
def _youku_text_value(value):
    if isinstance(value, str):
        return value.strip()
    if isinstance(value, dict):
        for key in ('title', 'text', 'name'):
            text = _youku_text_value(value.get(key))
            if text:
                return text
    return ''
...
def _youku_tag_titles(item):
    ...
    for obj in _youku_walk(item):
        if isinstance(obj, dict):
            t = _youku_text_value(obj.get('title'))
            if t:
                tags.append(t)
    return tags
...
async def youku_all_initial_data(include_short_drama=False):
    """优酷主频道 + 电视剧/动漫/电影/综艺频道，短剧按开关追加。"""
    async def one(ch, url):
        return (ch, url, await youku_initial_data(url))
    channels = list(YOUKU_CHANNELS)
    if include_short_drama:
        channels.extend(YOUKU_SHORT_DRAMA_CHANNELS)
    return await asyncio.gather(*(one(ch, url) for ch, url in channels))

def _youku_text_title(obj):
    return _youku_text_value(obj.get('text') if isinstance(obj, dict) else None)

def _youku_item_reason(item):
    return _youku_text_value(item.get('reason') if isinstance(item, dict) else None)

def _youku_item_top_mark(item):
    return _youku_text_value(item.get('topLeftMark') if isinstance(item, dict) else None)
